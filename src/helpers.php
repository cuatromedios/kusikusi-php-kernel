...
if (!function_exists('params_as_array')) {
    ...
    function process_querystring(
        ...
        foreach ($fields as $field) {
            ...
            if (!isset($select[$fieldParts['table']])) {
                $select[$fieldParts['table']] = [];
            }
        ...
        foreach (array_keys($select) as $relationName) {
            ...
            if (count($select[$tableName]) > 0 && $select[$tableName][0] != '*') {
                ...
                $query->with([
                    $relationName => function ($select) use ($dataFields, $relationName) {
                        $dataFields[] = 'id';
        ...
        if ($queryWhere !== null) {
            ...
            foreach ($filters as $filter) {
                $filterParts = preg_split('/(\:|\<|\>|\!:)/i', $filter, -1,
